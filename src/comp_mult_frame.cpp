...
//		ifs_target_cam >> target_frame_num;
	...
	if(!f_target_video){
		...
		cout << "target frame : " << target_frame_num << "[frame]"
			  << "time : " << target_frame_time << "[sec]" << endl;
	}
	cout << "done" << endl;
...
*/
	...
	while (1) {
		...
		cout << " pano_sd yaw : " << pano_sd.alpha << " pitch : " << pano_sd.beta
		     << " roll : " << pano_sd.gamma << endl;
		...
		double distanse = sqrt(pow(
				...
				// 近いものがあったらnear_sdを更新
				if (distanse < 0.5 && distanse < min) {
					min = distanse;
					//near_sd = pano_sds[i];
					cout << "detect near frame : " << n
							<< endl;
					//f_detect_near = true;
